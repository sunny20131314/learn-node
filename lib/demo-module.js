// require 的参数中不含有文件路径,Node会到模块的安装目录,去寻找已安装的模块
// 模块一旦被加载以后,就会被系统缓存, 如果第二次还加载该模块,就会返回缓存中的版本,
// 这意味着模块实际只会执行一次, 如果希望执行多次,可返回一个函数,然后多次调用该函数
...
// 但是不能直接给 exports及module.exports指向一个值(这样的话,之间的联系就木有了)
